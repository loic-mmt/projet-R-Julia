...
cleaning_pipeline <- function(data, min_year = 2000, max_year = as.integer(format(Sys.Date(), "%Y")), verbose = TRUE) {
  ...
  # Définir les colonnes requises (exemple basé sur des données courantes)
  CORE_COLUMNS <- c(
    "work_year", "experience_level", "employment_type", "job_title", 
    "salary", "salary_currency", "salary_in_usd"
  )
  
  # Étape 1 : Validation du schéma (CORRECTION APPLIQUÉE ICI)
  ...
  # Étapes restantes du pipeline (la numérotation des étapes est ajustée pour 13 étapes)
  
  # Étape 2 : Standardiser les noms de colonnes
  if (verbose) message("\n[2/13] Standardisation des noms de colonnes...")
  # NOTE : La fonction standardize_colnames n'est pas fournie, je suppose qu'elle existe.
  ...
  # NOTE : La fonction trim_whitespace n'est pas fournie, je suppose qu'elle existe.
  # data <- trim_whitespace(data)
  if (verbose) message(sprintf("  -> Espaces nettoyés"))
  
  # Étape 4 : Appliquer les types (enforce_types est définie ci-dessus)
  if (verbose) message("\n[4/13] Application des types de données...")
  data <- enforce_types(data)
  if (verbose) message(sprintf("  -> Types appliqués"))
  
  # Étape 5 : Supprimer les doublons (deduplicate_rows est définie ci-dessus)
  if (verbose) message("\n[5/13] Suppression des doublons...")
  data <- deduplicate_rows(data)
  if (verbose) message(sprintf("  -> Lignes restantes : %d", nrow(data)))
  
  # Étape 6 à 10 : Normalisations (Les fonctions ne sont pas définies, je les commente)
  ...
  # NOTE : La fonction convert_currency_to_usd est celle que nous avons testée précédemment.
