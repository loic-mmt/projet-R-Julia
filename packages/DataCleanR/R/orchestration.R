...
  # J'utilise to_snake_case si standardize_colnames est censée l'utiliser
...
 
  # Étape 4 : Appliquer les types (enforce_types)
  if (verbose) message("\n[4/13] Application des types de données...")
  **data <- enforce_types(data)** # Activation
  if (verbose) message(sprintf("  -> Types appliqués"))
 
  # Étape 5 : Supprimer les doublons (deduplicate_rows)
  if (verbose) message("\n[5/13] Suppression des doublons...")
  **data <- deduplicate_rows(data)** # Activation
  if (verbose) message(sprintf("  -> Lignes restantes : %d", nrow(data)))
 
  # Étape 6 à 10 : Normalisations
...
  if (verbose) message(sprintf("  -> Normalisé"))
 
  # Étape 12 : Valider les plages (validate_ranges)
  if (verbose) message("\n[12/13] Validation des plages de valeurs...")
  **data <- validate_ranges(data, min_year = min_year, max_year = max_year)** # Activation
  if (verbose) message(sprintf("  -> Lignes restantes : %d", nrow(data)))
 
  # Étape 13 : Convertir les devises (convert_currency_to_usd)
